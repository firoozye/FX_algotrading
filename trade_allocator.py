...
class TradeAllocator(object):
    ...
    def update_state(self, signals_ser, allocation_date):
        ...
        self.expected_gain_net_financing = (self.exp_gain / self.scale_all) - (
                self.funding_multiplier / self.scale_all) * (
                                                   np.heaviside(self.exp_gain, 0) * signals_ser[
    ...
    def _create_realized_returns_series(self, realized_gain: float):
        ...
        self.returns_ser.loc['real_short_gain'] = self.returns_ser['realized_gain'] + self.returns_ser[
            'short_financing_cost']
        self.returns_ser.loc['real_long_gain'] = self.returns_ser['realized_gain'] - self.returns_ser[
            'long_financing_cost']
    ...
    def set_cut_date(self, start_perf_msmt_date=settings.START_PERFORMANCE_MSMT):
        ...
        if len(cut_dates) == 0:
            ...
            self.first_date = (self.returns_frame.index[0] if (self.returns_frame.shape[0] > 0)
                               else settings.START_PERFORMANCE_MSMT)
        else:
    ...
    def _create_realized_returns_frame(self, realized_series):
        ...
        try:
            ...
            if not self.returns_frame.empty:
                ...
                try:
                    self.returns_frame.to_csv(
                        settings.ERROR_DIR + 'error_in_ta_returns_{}.csv'.format(four_digit_random_number))
                except:
            ...
            if not realized_frame.empty:
                ...
                try:
                    realized_frame.to_csv(
                        settings.ERROR_DIR + 'error_in_ta_realized_series_{}.csv'.format(four_digit_random_number))
                except:
